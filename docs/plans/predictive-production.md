...
4. The health-score card defaults to its original score and history. Its top-right **Forecast** button toggles only that card’s content to **Receipt outlook**, following the selected company and month. It shows 30/60/90-day checkpoints with separate forecasts explicitly unavailable, then the combined contraction and expansion probabilities for the exact three-calendar-month horizon, unavailable reasons and the model’s limitations.
