...
The open path belongs to the company-access empty state. The chairs belong to assistant connections and are also imported by the landing’s MCP section, reusing the same original file. Both empty-state images are decorative; the accompanying heading, instructions and action communicate the state without relying on the artwork. The larger landing image has a descriptive alternative.
