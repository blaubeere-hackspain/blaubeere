...
## Guide implementation and retest

The guide's decoupled flow is deployed: `list_companies` returns data only, then `render_company_picker` accepts selected IDs and owns the template. Its input is re-authorised against the account before any summaries are returned. Unit and production checks cover both tools, the output schema, anonymous discovery, and rejection of unassigned IDs.

Deployment [35486968856](https://github.com/blaubeere-hackspain/blaubeere/actions/runs/35486968856) passed. Fresh ChatGPT conversation `https://chatgpt.com/c/6aaf5550-d318-83eb-b6a0-cc40163d9894` called the new flow but still showed the app-loading error with zero widget iframes. A subsequent test pointing the compatibility alias at a legacy-MIME resource did not change the failure. Diagnostics [35487430941](https://github.com/blaubeere-hackspain/blaubeere/actions/runs/35487430941) show ChatGPT continued requesting the standard v4 resource, so this did not isolate legacy MIME support. Both fields were restored to the standard URI.

The same HTML was tested in a temporary local MCP Apps host: initialization, tool-result hydration, company switching, health/alert rendering, model-context updates and follow-up messages all worked. That host used clearly labelled test values; it does not establish native ChatGPT success. Production checks separately validate real authenticated data and company isolation.

The unresolved boundary is ChatGPT's resource loading before component execution. Following the official troubleshooting guide, the conversation, app/version IDs above, deployment runs and resource-fetch logs are the escalation evidence. No browser-side component error or rejected-field diagnostic is exposed, so a specific client or server defect is not yet established.
