...
The OAuth-protected `/mcp` endpoint exposes `get_company_health` with an exact `company_id` and optional `month` (`YYYY-MM`). It defaults to the latest month with company activity and returns the dated health state, issues, cash and payment metrics, original-currency totals, coverage and source provenance. `list_companies` lists only the signed-in account's memberships. `render_company_picker` takes IDs from that result, checks access again, and renders the selector in ChatGPT. Each health-data selection is also authorised on the server.

The widget follows the [official MCP Apps UI guide](https://developers.openai.com/plugins/build/chatgpt-ui): only `render_company_picker` declares `_meta.ui.resourceUri`, `resources/read` returns self-contained `text/html;profile=mcp-app`, and the iframe initializes the `ui/*` postMessage bridge before requesting company health through `tools/call`. The Next.js `/mcp` rewrite forwards to the existing Rust MCP process; the widget never receives an OAuth token or calls the database directly.

Protocol discovery and the allowlisted static HTML resources are public so ChatGPT can refresh its tool definitions and load a template without a user token. Every `tools/call`, including `list_companies`, still requires OAuth and company authorization. Historical widget URIs remain available for existing conversations. When changing the template, increment its URI, refresh the ChatGPT app's actions, verify the refresh succeeds, and test a fresh conversation. See [OpenAI's troubleshooting guide](https://developers.openai.com/plugins/deploy/troubleshooting). `cargo test -p blaubeere-mcp` checks anonymous discovery/template loading, authenticated data access, and rejection of unauthenticated tool calls and private resource paths.
