...
| MEDIUM | `apps/landing/app/page.tsx` — `#assistants` | MCP capabilities had no dedicated landing explanation. | A large shared oil painting on the left and three separated benefits on the right; stacked on phones. Copy covers cash outlooks, evidence and conditional plans, with compatibility and access limits beside the workspace link. | Visitors can understand what a connected assistant does before opening the app. |
...
- MCP landing section inspected in Chrome at 1800px and 390×844: loaded painting, three visible feature rows with separators, no horizontal overflow, and a working CTA to the dashboard. Landing TypeScript and production build pass with the shared artwork import. Production smoke checks now require all five landing paintings.
