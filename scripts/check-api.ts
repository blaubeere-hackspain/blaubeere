...
assert.ok(password, "Run bun run setup first, then start the services.");
const credentials = { email: process.env.BOOTSTRAP_EMAIL, password };
const login = await fetch(`${app}/api/auth/login`, { method: "POST", headers: { "Content-Type": "application/json", Origin: app }, body: JSON.stringify(credentials) });
