...
assert.ok(defaultingMarkup.includes("0.34 / 1") && defaultingMarkup.includes("width:40%") && defaultingMarkup.includes("width:20%") && defaultingMarkup.includes("not default probability"), "Defaulting shows the saved index and both sides on a fixed 0–1 scale");
...
assert.ok(fxMarkup.includes("Currency risk") && fxMarkup.includes("0.32 / 1") && fxMarkup.includes("width:32%") && fxMarkup.includes("1.21 points") && fxMarkup.includes("Maximum reduction: 5%"), "Currency risk shows the observed index, its amber bar and the score reduction");
...
assert.ok(fxUnknownMarkup.includes("FX evidence unavailable") && !fxUnknownMarkup.includes('class="fx-track"') && !fxUnknownMarkup.includes("0 / 1"), "An unknown FX index is missing evidence, never zero and never punished");
...
assert.ok(fxZeroMarkup.includes("0 / 1") && !fxZeroMarkup.includes("FX evidence unavailable"), "A known zero FX index is shown as 0 / 1, not as missing evidence");
...
assert.ok(!modelReason("castigo_fx_indice_0.5000").includes("NaN") && modelReason("castigo_fx_indice_0.5000").includes("0.5 / 1"), "A well-formed FX reason code stays readable");
