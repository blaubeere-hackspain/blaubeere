...
import { cashForecastAvailable, DailyCashPlot, MonthlyCashChart } from "../apps/app/components/monthly-cash-chart";
import { api, ApiError, returnPath } from "../apps/app/lib/api";
import { addDays, cents, date, money, monthlyTimeline } from "../apps/app/lib/format";
...
import type { CashMonth, CashProjection, ModelAssessment, ModelRecord } from "../apps/app/lib/types";
...
for (const text of ["Effective obligations (T6)", "€200", "Debt-service shortfall", "€30", "Debt multiplier reduction", "18 points", "40%", "0.75×", "× debt multiplier"]) assert.ok(v4Detail.includes(text), text);
...
assert.ok(!previewHealth.includes("Forecast") && !previewHealth.includes("Receipt outlook"), "The health card only displays observed scores and history");
...
assert.ok(previewHealth.includes("57.4") && previewHealth.includes("+5.2 points since 31 Jul") && previewHealth.includes('stroke-dasharray="4 4"'), "The landing preview uses the current Blau score history, including the selected month marker");
...
assert.ok(!flaggedHealth.includes("health-risk-alert") && !flaggedHealth.includes("Cash-flow health needs attention") && !flaggedHealth.includes("Review issue") && !flaggedHealth.includes("Provisional attention rules") && flaggedHealth.includes("31 Aug 2026"), "Keep the dated health score without the removed attention alerts");
const previewCash = preview.record.daily_cash[0];
assert.deepEqual([preview.company_id, preview.company_name, preview.record.as_of, previewCash.days.length, previewCash.income, previewCash.expense, previewCash.closing_balance], ["COMP_0318", "Blau, Corp.", "2026-08-31", 31, 75937.41, 29646.77, 112571.69]);
assert.equal(previewCash.days.reduce((sum, day) => sum + Math.round(day.income * 100) - Math.round(day.expense * 100), 0), 4629064, "The published preview daily cash reconciles to its month totals");
assert.deepEqual(preview.record.cash_projection.horizons.map(point => point.h), [30, 60, 90]);
assert.deepEqual(preview.record.health_projection.points.map(point => [point.h, point.date]), preview.record.cash_projection.horizons.map(point => [point.h, point.date]), "The preview forecasts use the same dated horizons for cash and health");
assert.equal(preview.record.cash_projection.as_of, preview.record.as_of);
assert.equal(preview.record.health_projection.as_of, preview.record.as_of);
...
const projection: CashProjection = { version: "cashflow_projection_v1", as_of: record.as_of, currency: "EUR", horizons: ([30, 60, 90] as const).map((h, index) => ({ h, date: addDays(record.as_of, h), saldo_corte_eur: 1000, entrada_esperada_eur: 100000, salida_esperada_eur: -100000 - index * 1000, flujo_neto_esperado_eur: -index * 1000, saldo_proyectado_eur: 1000 - index * 1000 })) };
const forecastDaily = { ...sameScale, days: [{ ...sameScale.days[0], date: record.as_of }] };
const forecastRow = { ...cashRecord, daily_cash: [forecastDaily], cash_projection: projection };
assert.equal(cashForecastAvailable(forecastRow), true);
assert.equal(Boolean(cashForecastAvailable({ ...forecastRow, cash_projection: null })), false);
assert.equal(cashForecastAvailable({ ...forecastRow, cash_projection: { ...projection, as_of: "2026-07-31" } }), false, "Never use a different cutoff’s forecast");
assert.equal(cashForecastAvailable({ ...forecastRow, cash_projection: { ...projection, horizons: projection.horizons.map(p => ({ ...p, saldo_proyectado_eur: null })) } }), false, "Unknown opening cash cannot become a projected zero balance");
...
const partialForecast = renderToStaticMarkup(createElement(DailyCashPlot, { series: forecastDaily, projection: { ...projection, horizons: projection.horizons.map(p => p.h === 60 ? { ...p, saldo_proyectado_eur: null } : p) } }));
assert.ok(!partialForecast.includes('data-forecast-horizon="60"') && (partialForecast.match(/data-series="forecast" d="([^"]*)"/)![1].match(/M/g) ?? []).length === 2, "Forecast gaps remain gaps");
...
assert.ok(defaultingMarkup.includes("34%") && defaultingMarkup.includes("width:40%") && defaultingMarkup.includes("width:20%") && defaultingMarkup.includes("not default probability"), "Defaulting shows the saved index and both sides as percentages");
for (const value of [null, undefined, NaN, Infinity, -1, 1.01]) assert.equal(formatIndex(value), "Not available");
for (const [value, expected] of [[0.09, "9%"], [0.083, "8.3%"], [0.105, "10.5%"], [1, "100%"]] as const) assert.equal(formatIndex(value), expected);
assert.equal(formatIndex(0), "0%", "A known zero is distinct from missing arrears evidence");
...
assert.ok(partialDefaulting.includes("Customer evidence only") && partialDefaulting.includes("Not available") && (partialDefaulting.match(/class="defaulting-track"/g) ?? []).length === 1, "An unknown side stays missing while a known zero keeps its bar");
const missingDefaulting = renderToStaticMarkup(createElement(DefaultingCard, { row: record }));
assert.ok(missingDefaulting.includes("Insufficient payment evidence") && !missingDefaulting.includes('class="defaulting-track"') && !missingDefaulting.includes(">0%<"));
...
assert.ok(fxIntervalMarkup.includes("50%") && fxIntervalMarkup.includes("minimum compatible penalty"), "An opaque-currency row shows the applied minimum penalty without a range");
assert.ok(!fxIntervalMarkup.includes("50% – 100%") && !/interval/i.test(fxIntervalMarkup) && !fxIntervalMarkup.includes("NaN") && !fxIntervalMarkup.includes("undefined"), "No min-max range or interval wording is ever rendered");
...
assert.ok(fxUnknownMarkup.includes("FX evidence unavailable") && !fxUnknownMarkup.includes('class="fx-track"') && !fxUnknownMarkup.includes(">0%<"), "An unknown FX index is missing evidence, never zero and never punished");
...
const fxIntervalUnknownMarkup = renderToStaticMarkup(createElement(DefaultingCard, { row: fxIntervalUnknownRow }));
const fxIntervalUnknownSection = fxIntervalUnknownMarkup.match(/<section class="defaulting-fx"[\s\S]*?<\/section><\/section>$/)![0];
assert.ok(fxIntervalUnknownSection.includes("FX evidence unavailable") && !fxIntervalUnknownSection.includes("Higher means"), "An interval row without an applicable value shows the missing-evidence paragraph");
assert.ok(!modelReason("castigo_fx_indice_0.5000").includes("castigo_fx") && modelReason("castigo_fx_indice_0.5000").includes("50%"), "Dynamic FX reason codes are redacted in readable text");
