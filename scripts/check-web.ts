...
assert.ok(!fxMarkup.includes("points per unit") && fxMarkup.includes("5%") && fxMarkup.includes("0.05"), "The FX footer states a percentage of the score derived from beta_fx, never points per index unit");
...
assert.ok(fxIntervalUnknownSection.includes("could not be valued this month") && !fxIntervalUnknownSection.includes("Higher means"), "An interval row without an applicable value shows the missing-evidence paragraph");
