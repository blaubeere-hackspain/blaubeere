...
import { Activity, ArrowDownRight, ArrowRight, Building2, CalendarDays, CircleHelp, FlaskConical, Layers3, LogOut, PanelLeft, Plug, ShieldCheck, TrendingUp, TriangleAlert, Wallet, X } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!selected) return;
    const controller = new AbortController(); setLoading(true); setError(""); setComparison(null); setPlan(undefined); setAssessment(null);
  ...
  return <div className="app-shell"><a href="#main" className="skip-link">Skip to cash outlook</a>
    <aside className="sidebar"><a className="brand" href="/dashboard"><span className="brand-mark" aria-hidden><i/><i/><i/><i/></span>blaubeere</a><div className="workspace-label"><span className="workspace-avatar"><Building2 size={18} aria-hidden/></span><div><strong>Finance workspace</strong><span>Private company access</span></div><ShieldCheck size={14} aria-hidden/></div>
      <span className="eyebrow nav-label">Workspace</span><nav aria-label="Workspace"><a className="nav-link active" href="/dashboard" aria-current="page"><Activity size={18} aria-hidden/>Cash outlook<span className="nav-indicator"/></a><button className="nav-link" onClick={() => setPlanning(true)} disabled={!company}><TrendingUp size={18} aria-hidden/>Explore a plan</button></nav>
    ...
    <div className="app-content"><header className="topbar"><div className="topbar-context"><span className="desktop-label"><PanelLeft size={17} aria-hidden/></span><span className="muted">Workspace</span><span className="breadcrumb-slash">/</span><strong>Cash outlook</strong></div><div className="topbar-actions"><span className="private-label"><ShieldCheck size={14} aria-hidden/>Private workspace</span><button className="icon-button mobile-control" onClick={() => setConnections(true)} disabled={!identity} aria-label="Connect assistant"><Plug size={18}/></button><button className="icon-button mobile-control" onClick={logout} disabled={logoutBusy || !identity} aria-label="Sign out"><LogOut size={18}/></button><a href="#evidence" className="icon-button" aria-label="Understand sources and assumptions"><CircleHelp size={19}/></a><span className="user-avatar desktop-label">FT</span></div></header>
      <main id="main" className="dashboard-main"><div className="page-heading"><div><div className="eyebrow mb-2">A little foresight goes a long way</div><h1>Your cash, in perspective.</h1><p className="muted mt-2">See what’s ahead. Understand why. Choose your next move.</p></div><button className="button" onClick={() => setPlanning(true)} disabled={!company}><TrendingUp size={17} aria-hidden/>Explore a plan<ArrowRight size={16} aria-hidden/></button></div>
        ...
        {loading && <div className="dashboard-skeleton" role="status"><span className="sr-only">Loading company cash outlook…</span><div className="skeleton-stats">{[1,2,3,4].map(n => <div key={n}/>)}</div><div className="skeleton-chart"/></div>}
        {!loading && companies?.length === 0 && <div className="empty-workspace"><Building2 size={32} aria-hidden/><h2>No company assessments yet</h2><p className="muted">Ask your administrator to grant company access and load an assessment.</p><button className="button secondary" onClick={() => setRetry(retry + 1)}>Refresh access</button></div>}
        {!loading && company && forecast && <>
          ...
          <section className="card chart-card"><div className="card-heading"><div><h2>The road ahead</h2><p className="muted small mt-1">Cash history and your {forecast.horizon_days}-day baseline forecast</p></div><span className="badge">{company.history_mode === "reconstructed" ? "Reconstructed history" : "As known at the cutoff"}</span></div>
