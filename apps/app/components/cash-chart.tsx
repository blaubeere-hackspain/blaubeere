...
export function CashChart({ company, forecast, plan }: { company: Company; forecast: Forecast; plan?: Plan }) {
  ...
  const pad = Math.max((high - low) * .14, 100_000);
  ...
  return <div className="cash-chart">
    ...
    <svg ref={svg} viewBox={`0 0 ${width} 290`} role="img" aria-labelledby="cash-chart-title cash-chart-desc" onPointerMove={event => {
      ...
      {[0, 1, 2, 3, 4].map(i => { const v = min + (max - min) * i / 4; return <g key={i}><line x1="76" x2={right} y1={y(v)} y2={y(v)} stroke="var(--line)"/><text x="62" y={y(v) + 4} textAnchor="end">{money(v, company.currency, true)}</text></g>; })}
      ...
      {[company.history[0]?.date, company.assessment_date, forecast.points[Math.floor(forecast.horizon_days / 2)].date, forecast.points.at(-1)!.date].filter(Boolean).map(d => <text key={d} x={x(d!)} y="278" textAnchor="middle">{date(d!)}</text>)}
    ...
    <div className="chart-inspector"><label htmlFor="forecast-day">Inspect day <span className="mono">{selected}</span><input id="forecast-day" type="range" min="0" max={forecast.horizon_days} value={selected} onChange={e => setDay(Number(e.target.value))}/></label><output htmlFor="forecast-day"><span>{date(point.date)}</span><strong className="num">{money(point.cash_cents, company.currency)}</strong>{plan && <span className="plan-value">Plan: {money(plan.forecast.points[selected]?.cash_cents ?? 0, company.currency)}</span>}</output></div>
