...
import { HealthScorePanel, healthForecastAvailable, modelNumber } from "./health-score-panel";
...
export function ModelDashboard({ data, scoreDate, demo = false, welcome }: { data: ModelAssessment; scoreDate?: string; demo?: boolean; welcome?: ReactNode }) {
  ...
  const canForecast = Boolean(row && (cashForecastAvailable(row) || healthForecastAvailable(row)));
  ...
  return <ChartMotion><div className="model-dashboard">
    ...
    {!row ? <section className="card metric-intro"><h2>{availableDates.length ? "No assessment for this date" : "No recorded activity yet"}</h2><p>{availableDates.length ? "Choose an available month above." : "No cash movements, invoices or model activity were recorded for this company."}</p></section> : <>
      ...
      <HealthScorePanel records={data.records} row={row} detail={Boolean(scoreDate)} forecast={forecast} action={!scoreDate && <button type="button" className="button secondary forecast-toggle" aria-pressed={forecast} aria-controls="health cash" disabled={!canForecast && !forecast} title={canForecast ? "Show the 30, 60 and 90-day cash and estimated health forecasts" : "Forecasts are unavailable for this month"} onClick={() => setForecast(!forecast)}><TrendingUp size={16} aria-hidden/>Forecast</button>}/>
      {!scoreDate && <MonthlyCashChart row={row} forecast={forecast} onCurrencyChange={() => setForecast(false)}/>}
      </div>
