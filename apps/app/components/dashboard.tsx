...
export function Dashboard({ demo, scoreDate }: { demo?: Demo; scoreDate?: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api<Identity>("/me", { signal: controller.signal }), api<CompanySummary[]>("/companies", { signal: controller.signal })]).then(([user, list]) => {
      ...
      if (requested && !list.some(c => c.id === requested)) {
        setSelected(""); setError("This company is not available in your workspace. Choose a company from the sidebar."); setLoading(false); return;
      }
      setSelected(requested ?? list[0]?.id ?? "");
  ...
  return <div className="app-shell" data-sidebar-collapsed={sidebarCollapsed}><a href="#main" className="skip-link">{scoreDate || model ? "Skip to health assessment" : "Skip to cash outlook"}</a>
    ...
    <div className="app-content"><header className="topbar"><div className="topbar-context">
      ...
      <main id="main" className="dashboard-main" tabIndex={-1}>{demo && <p className="demo-notice"><FlaskConical size={16} aria-hidden/>You’re exploring sample data. Try the charts, sources and example plans.</p>}{!scoreDate && !model && <><div className="page-heading"><div><div className="eyebrow mb-2">A little foresight goes a long way</div><h1>Your cash, in perspective.</h1><p className="muted mt-2">See what’s ahead. Understand why. Choose your next move.</p></div><button className="button" onClick={() => setPlanning(true)} disabled={!company || loading}><TrendingUp size={17} aria-hidden/>Explore a plan<ArrowRight size={16} aria-hidden/></button></div>
        ...
        {model && <div aria-busy={loading}><ModelDashboard key={model.company.id} data={model} scoreDate={scoreDate} companies={companies ?? []} onCompany={setSelected}/></div>}
        ...
        {company && forecast && !scoreDate && <div aria-busy={loading}>
          ...
          <section className="card health-card health-overview" id="health"><div className="card-heading"><div><h2>Daily financial health</h2><p className="small muted mt-1">Select a rating to read that day’s model explanation.</p></div><Activity size={18} className="muted" aria-hidden/></div><div className="health-overview-body"><div>{latestHealth && <><Link className="health-score-link" href={healthHref(company.id, latestHealth.date, Boolean(demo))} aria-label={`Explain health score ${latestHealth.score} on ${date(latestHealth.date, true)}`}><strong className="num">{latestHealth.score}</strong><span>/ 100</span><ArrowRight size={22} aria-hidden/></Link><p className="small muted">{date(latestHealth.date, true)} · {latestHealth.assessment?.health.label}</p><p className="small muted mt-2">{latestHealth.assessment?.health.note}</p></>}</div><div className="health-trend"><span className="eyebrow">Saved ratings</span><div>{health.map(point => <Link key={point.date} href={healthHref(company.id, point.date, Boolean(demo))} aria-label={`Explain health score ${point.score} on ${date(point.date, true)}`}><strong className="num">{point.score}</strong><small>{date(point.date)}</small></Link>)}</div></div></div></section>
