...
import type { Assessment, CompanySummary, Identity, ModelAssessment } from "../lib/types";
import { ModelDashboard } from "./model-dashboard";
import { ProxyAssessment } from "./proxy-assessment";
...
type DashboardAssessment = ModelAssessment | Extract<Assessment, { cash_planning_available: false }>;
...
export function Dashboard({ demo = false, scoreDate }: { demo?: boolean; scoreDate?: string }) {
  ...
  const [assessment, setAssessment] = useState<DashboardAssessment | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([demo ? Promise.resolve(null) : api<Identity>("/me", { signal: controller.signal }), api<CompanySummary[]>(`${prefix}/companies`, { signal: controller.signal })]).then(([user, list]) => {
      const imported = list.filter(company => company.data_mode === "challenge" || (!demo && company.data_mode === "synthetic"));
  ...
  useEffect(() => {
    ...
    api<DashboardAssessment>(`${prefix}/companies/${encodeURIComponent(selected)}/assessment`, { signal: controller.signal }).then(value => {
      if ("kind" in value ? value.kind !== "model" : demo || value.company.kind !== "proxy_only") throw new Error("No supported assessment is available for this company.");
      setAssessment(value);
    }).catch(error => { if (!controller.signal.aborted) handleError(error); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [selected, retry, prefix, demo]);
  ...
  const current = assessment?.company.id === selected ? assessment : null;
  const model = current && "kind" in current ? current : null;
  const proxy = current && !("kind" in current) ? current : null;
  ...
  return <div className="app-shell" data-sidebar-collapsed={sidebarCollapsed}><a href="#main" className="skip-link">Skip to financial health</a>
    ...
    <div className="app-content"><header className="topbar"><div className="topbar-context">
      ...
      <span className="current-view"><Activity size={16} aria-hidden/>{proxy ? "Operating deficit proxy" : scoreDate ? "Health assessment" : "Financial health"}</span>
      ...
      <main id="main" className="dashboard-main" tabIndex={-1}>
        ...
        <p className="refresh-status small" role="status">{loading && current && <><LoaderCircle className="spinner" size={14} aria-hidden/>Updating… Showing the previous assessment.</>}</p>
        {loading && !current && <div className="dashboard-skeleton" role="status"><span className="sr-only">Loading company health and source records…</span><div className="skeleton-chart"/><div className="skeleton-stats">{[1,2,3,4].map(n => <div key={n}/>)}</div></div>}
        {!loading && companies?.length === 0 && <PaintedEmptyState image={garden} title="No company assessments available."><p>{demo ? "The dataset has no companies to display yet." : "Your account does not have access to any company assessments yet. You can explore the published dataset in the demo."}</p>{!demo && <a className="button secondary" href="/demo">Explore company data<ArrowRight size={16} aria-hidden/></a>}</PaintedEmptyState>}
        {model && <div aria-busy={loading}><ModelDashboard key={model.company.id} data={model} scoreDate={scoreDate} demo={demo}/></div>}
        {proxy && <div aria-busy={loading}><header className="page-heading"><div><h1>{proxy.company.name}</h1><p className="muted mt-2">A synthetic retrospective estimate, with its evidence and limits.</p></div></header>{scoreDate && <p className="demo-notice">No published health score is available for this company. Showing its latest proxy assessment instead. <a href={overviewHref}>Back to overview</a></p>}<ProxyAssessment company={proxy.company} cashReason={proxy.cash_planning_reason}/></div>}
      </main>
    </div>
