...
export function Dashboard({ demo = false, scoreDate }: { demo?: boolean; scoreDate?: string }) {
  ...
  return <div className="app-shell" data-sidebar-collapsed={sidebarCollapsed}><a href="#main" className="skip-link">Skip to financial health</a>
    ...
    </div><div className="topbar-actions"><span className="private-label"><ShieldCheck size={14} aria-hidden/>{demo ? "Read-only demo" : "Private workspace"}</span>{demo ? <a className="topbar-link" href="/login">Sign in<ArrowRight size={14} aria-hidden/></a> : <span className="user-avatar" aria-label="Finance team">FT</span>}</div></header>
