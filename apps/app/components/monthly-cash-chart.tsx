...
export function DailyCashPlot({ series }: { series: CashMonth }) {
  ...
  const y = (value: number) => 250 - (value - min) / (max - min) * 220;
  ...
  return <>
    <div className="model-chart cash-flow-chart"><div ref={plot} className="cash-flow-plot" role="region" aria-label="Daily cash flow. Use left and right arrow keys to inspect a day; Escape returns to month totals." tabIndex={known.length ? 0 : undefined}
      ...
      {known.length ? <svg style={{ width }} viewBox={`0 0 ${width} 290`} role="img" aria-labelledby="daily-chart-title daily-chart-description" onPointerMove={event => { if (event.pointerType === "mouse") inspect(event); }} onPointerDown={inspect} onPointerLeave={() => setHovered(null)}>
        ...
        {hovered !== null && <rect x={left + hovered * slot} y="24" width={slot} height="232" rx="3" fill="var(--accent-soft)"/>}
        ...
        {days.map((day, index) => index === 0 || index === days.length - 1 || (index % 7 === 0 && index < days.length - 4) ? <text key={day.date} x={x(index)} y="279" textAnchor={index === 0 ? "start" : index === days.length - 1 ? "end" : "middle"}>{Number(day.date.slice(-2))}</text> : null)}
...
export function MonthlyCashChart({ row }: { row: ModelRecord }) {
  ...
  return <section className="card monthly-cash" id="cash" aria-labelledby="monthly-cash-title">
    ...
    {series ? <DailyCashPlot key={`${row.as_of}:${series.currency}`} series={series}/> : <p className="monthly-empty">{row.daily_cash == null ? "Daily cash movements have not been imported for this month." : "No cash accounts recorded for this company this month."}</p>}
  </section>;
}
