  ...
  return <Dialog open={open} onClose={onClose} className="planning-dialog" titleId="planner-title">
    ...
    <div className="panel-body"><p className="muted mb-6">Explore conditional changes for {company.name}. Your observed health and source records stay unchanged.</p>
      <form onSubmit={compare} onChange={() => setComparison(null)} aria-busy={busy}>
        ...
        <div className="planner-submit"><p className="small muted"><FlaskConical size={16} aria-hidden/>Two candidate combinations. Explicit assumptions.</p><button className="button" disabled={busy}>{busy ? <><LoaderCircle className="spinner" size={16} aria-hidden/>Comparing plans…</> : <>Compare plans<ArrowRight size={16} aria-hidden/></>}</button></div>
      </form>
