...
Do not set `ASSESSMENT_FILE` to this proxy export: current API/MCP accept only the cash fixture shape with numeric opening cash/buffer and reject retired `kind`/`predictive` fields. The cash response no longer contains `cash_planning_available`, `cash_planning_reason`, or a company `kind`/`predictive` discriminator. Imported scoring-v4 assessments keep their separate `kind: "model"` contract unchanged.
