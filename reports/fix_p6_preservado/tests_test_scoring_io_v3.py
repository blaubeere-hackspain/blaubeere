...
class ScoringIoV3Test(unittest.TestCase):
    ...
    def test_summary_counts_add_up_to_total(self):
        ...
        self.assertEqual(sum(summary['reparto_confidence'].values()),
                         summary['total_companies'])
        # A tiene un unico cobro y ningun pago operativo demostrado: con la
        # guarda sobre P6 pierde la nota (antes puntuaba con el bug de la
        # nota 100 sin pagos). B no tiene actividad ni flujos: cae en
        # sin_flujos_observados_en_la_ventana.
        self.assertEqual(summary['n_con_nota'], 0)
        self.assertEqual(summary['n_sin_nota_por_pagos_operativos_no_demostrados'], 1)
        self.assertEqual(summary['n_sin_nota_por_sin_flujos_observados_en_la_ventana'], 1)
        self.assertEqual(summary['n_sin_nota_por_falta_de_actividad'], 0)
