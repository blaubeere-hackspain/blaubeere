...
Entradas consumidas (todas point-in-time, no se modifican):
  ...
  - data/clean/transactions.parquet + data/interim/tx_flow_class.parquet:
    base de movimientos para recomputar C y P.
...
Motivos de no-nota (resumen del corte): sin_flujos_observados_en_la_ventana
(c6 = 0 y t6 = 0 en la ventana, incluye las empresas sin actividad) y
pagos_operativos_no_demostrados (p6 <= 0 en la ventana, guarda sobre P6).
...
D (servicio de deuda) desconocido NO baja la confianza declarada aqui; el
motor lo conserva como reason informativo sin deducir nivel.
...
def assess_at_v3(con, as_of, company_id=None, *, k=DEFAULT_K, alpha=DEFAULT_ALPHA,
    ...
    for company, group in selected.items():
        ...
        if health is None:
            if 'sin_flujos_observados_en_la_ventana' in engine['reasons']:
                motivo = 'sin_flujos_observados_en_la_ventana'
            elif 'pagos_operativos_no_demostrados' in engine['reasons']:
                motivo = 'pagos_operativos_no_demostrados'
    ...
    summary = {
        ...
        'n_sin_nota_por_sin_flujos_observados_en_la_ventana': sum(
            row['_motivo_sin_nota'] == 'sin_flujos_observados_en_la_ventana'
            for row in assessments),
        'n_sin_nota_por_pagos_operativos_no_demostrados': sum(
    ...
    assert (summary['n_con_nota'] + summary['n_sin_nota_por_falta_de_actividad']
            + summary['n_sin_nota_por_sin_flujos_observados_en_la_ventana']
            + summary['n_sin_nota_por_pagos_operativos_no_demostrados']) == summary['total_companies']
...
def _public_summary(result):
    ...
    return {
        ...
        'n_sin_nota_por_sin_flujos_observados_en_la_ventana':
            summary['n_sin_nota_por_sin_flujos_observados_en_la_ventana'],
        'n_sin_nota_por_pagos_operativos_no_demostrados':
            summary['n_sin_nota_por_pagos_operativos_no_demostrados'],
