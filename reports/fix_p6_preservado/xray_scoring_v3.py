...
inventa ningun numero):
  ...
  2. C6 = 0 y T6 = 0: nada de flujos observados en la ventana
     -> 'sin_flujos_observados_en_la_ventana'. Sustituye al antiguo
     'denominador_nulo', que con la guarda de P6 queda inalcanzable.
  3. P6 <= 0: ningun pago operativo demostrado en la ventana
     -> 'pagos_operativos_no_demostrados'. La guarda es sobre P6 y NO sobre
     T6: el colchon_aplicable entra en numerador y denominador y podria
     enmascarar T6 = 0 (antes daba H = 100 automatico). D6 = 0 es legitimo y
     NO bloquea por si solo.
Con la guarda de P6, si hay nota entonces T6 > 0 y el denominador es
positivo por construccion.
...
LIMITACIONES = (
    ...
    'Sin pagos operativos demostrados en la ventana no hay nota '
    '(pagos_operativos_no_demostrados); un servicio de deuda cero no '
    'bloquea por si solo.',
)
...
def score_company(company_id, group_id, monthly_rows, as_of, *,
    ...
    if not any_activity:
        ...
        if c6 == 0.0 and t6 == 0.0:
            reasons.append('sin_flujos_observados_en_la_ventana')
    elif c6 == 0.0 and t6 == 0.0:
        # Nada de flujos observados en la ventana: sin nota. Sustituye al
        # antiguo 'denominador_nulo', logicamente inalcanzable desde la
        # guarda de P6 (p6 > 0 implica t6 > 0 implica denominador > 0).
        health_score = None
        h_antes_de_mora = None
        penalizacion = 0.0
        reasons.append('sin_flujos_observados_en_la_ventana')
    elif p6 <= 0.0:
        # Guarda sobre P6, NO sobre T6: si T6 = 0 el shrinkage aporta
        # k*R_hist = k al numerador y k al denominador y daba H = 100
        # automatico. El colchon_aplicable entra en numerador y denominador
        # y podria enmascararlo, por eso la condicion no mira T6.
        # D6 = 0 es legitimo y NO bloquea por si solo.
        health_score = None
        h_antes_de_mora = None
        penalizacion = 0.0
        reasons.append('pagos_operativos_no_demostrados')
    else:
    ...
    # nivel por cada limitacion conocida: ventana parcial, cobros o pagos
    # desconocidos en meses activos, mora no observable y colchon no estimado.
    # D desconocido NO resta nivel: se conserva como reason informativo
    # (d_eur_desconocido_en_N_meses) pero no entra en la deduccion, porque el
    # servicio de deuda conservador del martillo de deuda anula el mes entero
    # ante cualquier salida ambigua y colapsaria la confianza por diseno.
    if active_window:
    ...
    # La deduccion solo mira C y P; D desconocido queda como reason
    # informativo en missing_counts/reasons, sin restar nivel.
