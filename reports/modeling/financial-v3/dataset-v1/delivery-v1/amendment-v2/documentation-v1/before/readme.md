> **Current delivery policy — authorized single-cache amendment (2026-09-20).** The user explicitly authorized treating only `apps/app/tsconfig.tsbuildinfo` as generated and regenerable. This does **not** recover its lost historical bytes or make original whole-worktree integrity true. The new pinned amendment verifies the other **3,025 historical bindings exactly**, preserves the original STOP/recovery evidence and scientific artifact inventories, and separately binds current application/helper/test sources. No other exception, scientific change, or security-policy change is allowed. Scientific utility remains failed; whole Goal v3 is not complete.

Current verification command: `.venv/bin/python -B scripts/financial_v3_dataset_delivery_amendment.py verify`. The decision, current/final source receipts, command logs and scoped technical results are under `reports/modeling/financial-v3/dataset-v1/delivery-v1/amendment-v2/`. Each check records the actual compiler-cache hash, size and version; old expected/current/failed-recovery hashes remain explicit. Final technical acceptance is separate from scientific utility and pending independent parent review. The original `financial_v3_dataset_delivery.py verify` still fails as historical evidence and is **not** a current green-check command. Legacy whole-worktree Python snapshot assertions remain unchanged and are not claimed passing; selected pure fixture suites and exact counts are listed in the new verification report, not substituted for all historical discovery.

> **Historical delivery STOP — integrity unresolved before the explicit cache amendment.** The application/browser checks passed, but the final preservation check detects regenerated, historically sealed `apps/app/tsconfig.tsbuildinfo`. Git does not track this compiler cache and its original bytes were not captured. No seal was changed and no mismatch is waived: `.venv/bin/python -B scripts/financial_v3_dataset_delivery.py verify` currently fails. See `delivery-v1/source-after-blocked.json` and `delivery-v1/verification.json` for current hashes, all differences and required parent resolution. V3T8 is not accepted complete.

> **Current V3 delivery — retrospective integration, not goal completion.** `financial_health_v3` now reads the real sealed dataset-v1 and model-v1 exports in the existing dashboard/API/MCP. **Internal retrospective utility FAILED both receipt proxies; persistence baseline won.** Validation: 28 labeled / 345 eligible origins, 8 groups; contraction AP 0.443 / Brier 0.211, expansion AP 0.410 / Brier 0.225; recall 0 at 0.6. Q4 has insufficient classes, no forecast. Full health remains null, automatic alerts disabled. Receipt-only probabilities do not validate general financial improvement, all six questions, or anticipation.

### Load the sealed V3 delivery

Use process-only `ASSESSMENT_FILE="$PWD/reports/modeling/financial-v3/dataset-v1/index.json"` for each Rust service. Keep the fixed local `manifest.json`, `schema.json`, `policy.json`, `profiles/` and `model-v1/` sibling structure. Dataset manifest SHA256 is `e360ef33ba7cae39aa9c94d0f8e778f49c38a3985f77053a2f53bd4ae5ef8a44`; model manifest SHA256 is `71eae8fef2ab3466fa2545d1c38f57977508cb79b7c051280b59e02253925428`. Startup caches small index/schema/report metadata, not company shards. API/MCP company authorization precedes per-company shard reads, which enforce the index/hash chain. No runtime Python, model pickle, inference, events or outcome labels.

`GET /api/companies/{id}/assessment?view=retrospective&as_of=2025-12-31` or MCP `get_cash_outlook({company_id, view: "retrospective", as_of: "2025-12-31"})` selects a retrospective closed month, not strict known-on evidence. Other views and strict requests reject. Responses expose exact current financial point and filtered history; model probabilities are null before January 2026 and for 208 final-test product-only companies. Reverse cash is separately labelled future-anchor reconstruction with retrieval/anchor dates; no raw anchor balance or latest debt context is delivered. The experiment report is later methodology, never historical evidence. Flow/service/AP/document money is EUR units; reconstructed cash is account-native, not company reporting currency. V3 never enables cash planning. Existing cash/v1/v2 modes remain unchanged in purpose.

```sh
.venv/bin/python -B scripts/financial_v3_dataset_delivery.py verify
bun run check
cargo build --offline --workspace
FINANCIAL_V3_ASSESSMENT_FILE="$PWD/reports/modeling/financial-v3/dataset-v1/index.json" cargo test --offline --workspace financial_v3 -- --ignored
TRAJECTORY_ASSESSMENT_FILE="$PWD/reports/modeling/trajectory-v2/assessment/companies.json" cargo test --offline --workspace actual_trajectory -- --ignored
bun run test:financial-v3:e2e
```

New verification/evidence is isolated under [dataset-v1/delivery-v1](reports/modeling/financial-v3/dataset-v1/delivery-v1/). Its fixed transition preserves exact historical app bytes under `source-before`, checks all 3,026 historical bindings and binds the delivered application separately. Legacy whole-worktree checkers and the frozen `test_all_input_and_protected_hashes_after` assert old application hashes: **they are not claimed passing on this modified application** and were not edited. Use the delivered scientific snapshot check plus current app regressions, not scientific model/evaluation replay. The new browser helper covers V3 plus cash/proxy/trajectory using fresh local identities and sandboxed existing Chromium; it writes only new delivery evidence, not old verification directories. Existing historical E2E commands below write older report locations and are retained as history, not the recommended V3 verification command.

The authorized internal-retrospective experiment uses the existing 24 months, superseding the former external-data prerequisite for this experiment; its negative/insufficient scientific results remain blockers to overall Goal v3. Full-company [0,100] is uninformative, not drawn as confident health. The operating component interval is explicitly not whole-company health.

> **Historical pre-integration v3 note — source audit and development design, not a new validated score.** [Audit evidence](reports/modeling/financial-v3/audit-summary.json) and [proposed financial/API contract](reports/modeling/financial-v3/design-proposal.md) are isolated from v1/v2. Incomplete cash, debt or settlement evidence requires a null headline; reconstructed retrospective balances cannot become earlier as-of signals. No untouched confirmatory test is identified: all 50 v1 final-test groups stay excluded, the v2 reserve is consumed, and new future/external data is required. No model, labels, evaluation or API/app implementation was run for this phase; old artifacts/results and the delivery documented below remain intact. Read-only preservation check: `.venv/bin/python -B scripts/financial_v3_audit.py --check`.
