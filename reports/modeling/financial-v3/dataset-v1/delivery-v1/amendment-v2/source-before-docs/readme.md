> **Delivery STOP — integrity unresolved.** The application/browser checks passed, but the final preservation check detects regenerated, historically sealed `apps/app/tsconfig.tsbuildinfo`. Git does not track this compiler cache and its original bytes were not captured. No seal was changed and no mismatch is waived: `.venv/bin/python -B scripts/financial_v3_dataset_delivery.py verify` currently fails. See `delivery-v1/source-after-blocked.json` and `delivery-v1/verification.json` for current hashes, all differences and required parent resolution. V3T8 is not accepted complete.
