...
impl FinanceTools {
    ...
    #[tool(
        description = "Read an authorised company assessment. Cash-mode amounts are integer cents. Proxy-only assessments return forecast null and cash_planning_available false, with synthetic reconstructed operating-deficit probability evidence over three calendar months. Display estimates only when accepted and eligible; this is not calibrated, default risk or health. Empty proxy cash history means unknown, not no activity. Operating trajectories accept as_of closed months September 2024–August 2026, filtered server-side with no future case annotations. Anticipation not validated; descriptive review only, automatic predictive alerts disabled. Signals are retrospective simulations, not historically issued live alerts; later methodological reports were not known at the cutoff.",
        annotations(
            read_only_hint = true,
            destructive_hint = false,
            idempotent_hint = true,
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Ok(path) = std::env::var("ASSESSMENT_FILE") {
        state.companies = Arc::new(finance::load_file(std::path::Path::new(&path))?);
    }
