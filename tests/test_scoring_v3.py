...
class ScoringV3Test(unittest.TestCase):
    ...
    # historial (y el caso del bug de la nota 100, que no produce None
    # hasta que se reintroduzca la guarda de P6).
    def test_no_activity_is_the_only_none(self):
        ...
        # Sin pagos operativos (p=None) y sin servicio de deuda (d=None):
        # el bug pre-fix da nota 100 sin pagos: el defecto conocido que el
        # fix de P6 bloquea (guarda sobre P6, no sobre T6).
        con_una_activa = rows + [row(4, c=100.0, p=None, d=None, colchon=0.0)]
        sin_pagos = self.score(con_una_activa, k=0.0)
        self.assertAlmostEqual(sin_pagos['health_score'], 100.0)
        self.assertNotIn('denominador_nulo', sin_pagos['reasons'])
        ...
        self.assertNotIn('denominador_nulo', con_nota['reasons'])

    # 13. Denominador nulo (c6=0 y t6=0): None con reason 'denominador_nulo'.
    def test_null_denominator(self):
        result = self.score([row(1, c=0.0, p=0.0, d=0.0, colchon=0.0)], k=0.0)
        self.assertIsNone(result['health_score'])
        self.assertIn('denominador_nulo', result['reasons'])
    ...
    def test_r_hist_undefined_omits_k_term(self):
        ...
        # Sin importes conocidos c6=0 y t6=0: denominador nulo, no hay nota,
        # con k grande ni con k=0.
        self.assertIsNone(con_k['health_score'])
        self.assertIsNone(sin_k['health_score'])
        self.assertIn('denominador_nulo', con_k['reasons'])
